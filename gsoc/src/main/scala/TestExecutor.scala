...
class TestExecutor(settings: TestSetting)(implicit cfg: Configuration) {
  def execute() {
    remoteActor(settings.serverport, 'server) {
      loop {
        react {
          case e => sender ! e
        }
  ...
  class ClientActor(runId: Int, latch: CountDownLatch) extends Actor {
    val nodes = settings.nodes.map(host => Node(host, settings.serverport).canonicalForm).toArray
